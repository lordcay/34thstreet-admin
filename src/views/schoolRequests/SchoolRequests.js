...
import {
  fetchSchoolRequests,
  fetchSchoolRequestStats,
  approveSchoolRequest,
  denySchoolRequest,
} from 'src/api/schoolRequests'
...
export default function SchoolRequests() {
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error(e)
      alert('Failed to load school requests.')
    } finally {
      setLoading(false)
    }
  }
    ...
    if (searchTerm.trim()) {
      ...
      result = result.filter(
        (r) =>
          ...
          r.schoolEmail?.toLowerCase().includes(term) ||
  ...
  const handleDeny = async () => {
    ...
    try {
      await denySchoolRequest(denyTargetId, denyReason || undefined)
      setDenyModal(false)
      await load()
    } catch (e) {
      console.error(e)
    ...
    <>
      ...
      <CCard className="mb-4">
        ...
        <CCardBody>
          ...
          {filtered.length === 0 ? (
            <CAlert color="info">No school requests found.</CAlert>
          ) : (
              ...
              <CTableBody>
                {filtered.map((req) => (
                  <CTableRow key={req._id || req.id}>
                    ...
                    <CTableDataCell>{req.schoolEmail}</CTableDataCell>
                    <CTableDataCell>{req.program}</CTableDataCell>
                    <CTableDataCell>
        ...
        <CModalBody>
          {selectedRequest && (
            <CListGroup flush>
              ...
              <CListGroupItem>
                <strong>School Email:</strong> {selectedRequest.schoolEmail}
              </CListGroupItem>
              <CListGroupItem>
              ...
              <CListGroupItem>
                ...
                <CBadge color={selectedRequest.schoolEmailVerified ? 'success' : 'danger'}>
                  {selectedRequest.schoolEmailVerified ? 'Yes' : 'No'}
                </CBadge>
              </CListGroupItem>
              <CListGroupItem>
